...
public sealed class WebhookService : IWebhookService, IHostedService
{
    ...
    {
        if (!_webhookHandler.ValidateWebhookRequest(jsonBody, secretTokenHeader, _options.SecretToken))
        {
            _logger.LogWarning("Rejected webhook request: secret token mismatch");
            return null;
        }
